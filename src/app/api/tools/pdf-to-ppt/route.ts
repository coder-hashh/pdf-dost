...
export async function POST(request: Request) {
  ...
  try {
    ...
    outputPath = await libreofficeConvert(inputPath, processedDir, "pptx")
    ...
    await logActivity(
      userId,
      "pdf_to_ppt",
      `Converted PDF to PowerPoint presentation: ${file.name}`,
      request
    )
    ...
    const message = error instanceof Error ? error.message : "Failed to convert PDF to PowerPoint"
